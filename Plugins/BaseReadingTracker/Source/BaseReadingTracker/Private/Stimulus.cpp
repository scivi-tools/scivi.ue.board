...
AStimulus::AStimulus(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	...
	static ConstructorHelpers::FClassFinder<UUserWidget> CanvasWidgetClass(TEXT("/Game/UI_Canvas"));
...
{
	Super::BeginPlay();
	GetWorld()->GetAuthGameMode<AReadingTrackerGameMode>()->NotifyStimulustSpawned(this);
...
//const TArray<const UAOI*>& AStimulus::GetSelectedAOIs() const &
//{
//	return SelectedAOIs;
//}

TArray<const UAOI*>& AStimulus::GetSelectedAOIs() &
{
	return SelectedAOIs;
}
			...
			if (selectedAOI)
			{
				...
				if (selectedAOI->audio_desciptions.Num() > 0)
				{
					if (selectedAOI->last_played_description >= selectedAOI->audio_desciptions.Num() - 1)
						selectedAOI->last_played_description = -1;
					FString path = selectedAOI->audio_desciptions[++selectedAOI->last_played_description];
					GM->informant->PlaySound(path);
				}
			}
		}
